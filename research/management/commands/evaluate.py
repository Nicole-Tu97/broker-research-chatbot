...
class Command(BaseCommand):
    ...
    def render_report(self, results) -> str:
        ...
        if b:
            ...
            L.append(f"plus one planted prompt-injection probe outside the golden set (a document seeded with")
            L.append(f"hidden instructions and a canary word) — {n_calls} live calls in total. The metrics below")
            L.append(f"score each question's most recent answer once; per-run raw numbers are archived in")
            L.append(f"`eval/results.json`.\n")
            L.append("**How answers are graded — preset rules, never an LLM judging an LLM.** Every")
            L.append("golden-set question was written with its grading rule attached, fixed before any")
