...
class Command(BaseCommand):
    ...
    def render_report(self, results) -> str:
        ...
        if r:
            ...
            if has_ag:
                ...
                if dp.get("agentic") is not None and dp.get("hybrid") is not None and dp["agentic"] < dp["hybrid"]:
                    L.append(f"**One weak spot to note.** `deep_page_recovery` is the weakest type on the agentic")
                    L.append(f"column ({dp['agentic']}) — and there, single-shot hybrid ({dp['hybrid']}) beats the agent. The")
                    L.append("reason is a deliberate cost-saving default: the agent starts from each report's first")
                    L.append("page and stops as soon as what it has read looks sufficient. On deep-page questions the")
                    L.append("first page often holds only a summary, so the agent answers from it and never retrieves")
                    L.append("the deeper page that carries the exact figure — while single-shot hybrid, which does")
                    L.append("not stop early, brings that page back in its top 10. (A few of these misses are the")
                    L.append("agent answering correctly from another valid page that the fixed answer key does not")
                    L.append("credit.) The improvement path: keep the single-shot hybrid results as a floor, so the")
                    L.append("agent's early stop can only add pages, never lose them — hybrid alone already scores")
                    L.append(f"{dp['hybrid']} on this type — and the stopping threshold itself can be tuned.\n")
        ...
        if b:
            ...
            L.append(f"recent answer once. Per-run raw numbers are archived in `eval/results.json`.\n")
