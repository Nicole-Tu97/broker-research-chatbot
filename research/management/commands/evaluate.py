...
class Command(BaseCommand):
    ...
    def render_report(self, results) -> str:
        ...
        if r:
            ...
            n_golden_s = f"{n_golden} questions" if n_golden else "124 questions"
            L.append(f"- **{n_golden_s}, each with a fixed answer key** (`eval/golden_set.json`): the pages")
            L.append("  that hold the answer, the facts the answer must state, a forbidden text pattern for")
            L.append("  questions that must be declined, and — for figure questions — a hand-drawn figure box.")
            L.append("- **The answer keys were drafted by an LLM, then checked and double-checked:** every")
            L.append("  expected fact was verified by code to appear on the expected page, and I manually")
            L.append("  reviewed a random sample of the keys. The chatbot under test never saw them.")
            L.append("- **One set feeds both tests.** The retrieval test uses the page keys")
            L.append(f"  ({n_items} questions have them); the behavior test uses the fact, pattern and box keys")
            L.append("  on every question.")
            L.append("- **Grading is rule-based against these fixed keys** (rules below), so the same set")
            L.append("  re-scores any future version of the system the same way.\n")
