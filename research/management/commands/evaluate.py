...
class Command(BaseCommand):
    ...
    def render_report(self, results) -> str:
        ...
        if r:
            ...
            L.append("and — for figure questions — a hand-drawn figure box. The retrieval test uses the")
            L.append(f"page keys ({n_items} questions have them); the behavior test uses the fact, pattern")
            L.append("and box keys on every question. Nothing is trained on this set — it is a fixed ruler,")
            L.append("the same questions measured along different dimensions.\n")
            L.append("**Where the answer keys come from.** Each key was written during development by")
            L.append("reading the source page, then checked by code: the expected page must exist, and the")
            L.append("expected fact must actually appear in that page's stored text. The chatbot never saw")
            L.append("the keys and had no part in writing them — the key and the answer under test come from")
            L.append("different places, which is what makes the comparison a test. One shared dependency")
            L.append("remains: keys and chatbot both read the same page transcriptions, so a transcription")
            L.append("error would affect both; transcription accuracy is measured separately (DESIGN.md §3).\n")
            L.append("## How answers are graded — preset rules, never an LLM judging an LLM\n")
            L.append("Every golden-set question was written with its grading rule attached, fixed before any")
            L.append("testing. There are three kinds of rule:\n")
            L.append("1. **Must contain** — exact facts the answer has to state (e.g. `$240`, `+20%`, a")
            L.append("   date). Graded by searching the answer text for those strings, with number formats")
            L.append("   normalized first (thousands separators, scale words, multiplication signs).")
            L.append("2. **Must NOT contain** — a text pattern that would betray a made-up answer. E.g. a")
            L.append("   question about a year the library does not cover forbids any 2–3 digit dollar")
            L.append("   figure: stating one means the system invented a price target instead of declining.")
            L.append("3. **Box match** — for figure questions, the hand-drawn rectangle on the source page")
            L.append("   that the returned crop must overlap (IoU ≥ 0.5).\n")
            L.append("One universal rule applies on top, to every answer: each number a citation carries")
            L.append("must actually appear on the cited page's text (the ✓/⚠ badge check). The chatbot's")
            L.append("answers are compared against these preset rules by a plain script — string search,")
            L.append("number comparison, rectangle overlap. No LLM grades another LLM's output anywhere,")
            L.append("so every score is exactly reproducible.\n")
            ...
            L.append(f"once per configuration (each returning")
            L.append(f"its top 10 pages — a slightly wider window than the live tool's default of 8, which")
            L.append(f"the agent can raise). The score per question is the share of its answer pages that")
            ...
            L.append("- `cross_broker_comparison` — needs pages from several documents (several brokers)")
            ...
            if has_ag:
                ...
                if dp.get("agentic") is not None and dp.get("hybrid") is not None and dp["agentic"] < dp["hybrid"]:
                    L.append(f"**One weak spot to note.** On `deep_page_recovery`, single-shot hybrid ({dp['hybrid']}) beats")
                    L.append(f"the agent ({dp['agentic']}). **The cause is the agent's cost-saving default: it reads each")
                    L.append("report's first page and stops as soon as that looks sufficient.** On deep-page")
                    L.append("questions the first page is only a summary, so the agent never fetches the deeper page")
                    L.append("with the exact figure — while single-shot hybrid, which never stops early, does. (A few")
                    L.append("misses are the agent answering correctly from another valid page the fixed answer key")
                    L.append("does not credit.) **Fix: keep the single-shot hybrid results as a floor** — the agent's")
                    L.append(f"early stop can then only add pages, never lose them (hybrid alone scores {dp['hybrid']} here) —")
                    L.append("and the stopping threshold itself can be tuned.\n")
        ...
        if b:
            ...
            L.append(f"{n_calls} live calls in total. The metrics below score each question's most recent")
            L.append(f"answer once; per-run raw numbers are archived in `eval/results.json`.\n")
            ...
            L.append(f"  broker the library does not cover); the system must decline. \"Answered anyway\" is")
            L.append(f"  detected mechanically, by a per-question forbidden text pattern — e.g. for a question")
            L.append(f"  about 2023 targets, any dollar figure in the answer counts as answering; a decline")
            L.append(f"  contains none: {ab_t - ab_p}/{ab_t} answered anyway (threshold = 0 → **{'PASS' if ab_p == ab_t else 'FAIL'}**)")
