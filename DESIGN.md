...
| **Verification** | Fixed-rule checks (no LLM) at two moments. During ingestion — the offline step that turns PDFs into the knowledge base — a numeric cross-check flags any transcribed number that does not exist in the page's own text layer. At answer time, every citation gets a grounding badge (✓/⚠) and, if a newer note from the same broker exists, a recency label. | This is the deterministic-checks box in the diagram: a **product feature that runs on every live answer** — distinct from the offline evaluation in §5, which grades the whole system against a golden set. Numbers are trusted because they are checked, not because the model said so. |
