...
|---|---|---|
| **Parsing** | Every page is **converted into text**: one markdown transcription that describes everything on the page — the prose, the tables (rebuilt as markdown tables), and what the charts show. Two inputs feed a single model call: the rendered full-page image, so vision can read what exists only as pixels, and the PDF's native text layer, so every number stays exactly as printed. | Charts and scanned slides become searchable text without losing exact figures. Both the prompt and the render resolution were set by a benchmark, not guessed (`bench/`: 20 human-verified pages × DPI tiers) — it caught the "blank cells filled with 0" failure the prompt now forbids, and showed that more resolution is not safer: 150 DPI hallucinated on the keynote while 72 passed. |
