...
| **Embedding** | text-embedding-3-large, one vector per page, requested at **1,024 dimensions** instead of the native 3,072 (the model is Matryoshka-trained: its leading dimensions carry most of the signal, so truncation is a supported, graceful trade). | Cross-lingual vector space (measured end-to-end, §4.1). The cut is forced, not cosmetic: pgvector's HNSW index caps the `vector` type at 2,000 dimensions, so 3,072 could not be indexed at all. Quality cost is small by the vendor's own published benchmarks (roughly half an MTEB point from 3,072 to 1,024; 1,024-d 3-large still outscores the previous-generation model at full width), while the index shrinks to a third and every distance computation gets 3× cheaper. API cost is per token, so the choice saves nothing on spend — it buys index size and speed. Not A/B tested: 1,024 vs 1,536 (both under the cap); 1,024 was taken as the standard cut with margin. |
